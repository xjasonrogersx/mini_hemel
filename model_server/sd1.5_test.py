#!/usr/bin/env python3
"""Send one Stable Diffusion 1.5 img2img request through RabbitMQ."""
...
def request_sd15(
    ...
    print(f"Sent Stable Diffusion 1.5 request to queue {queue}; waiting for response...")
    ...
    if not result.get("ok"):
        raise RuntimeError(
            result.get("error", "Stable Diffusion 1.5 worker returned an error")
        )
...
def main() -> None:
    ...
    parser.add_argument("image", type=Path, help="Input image for img2img")
    parser.add_argument(
        "--prompt",
        default=(
    ...
    parser.add_argument(
        "--negative-prompt",
        default=(
            "changed camera angle, changed buildings, changed cars, warped geometry, "
            "extra objects, text, watermark, blur"
        ),
        help="Negative image generation prompt",
    )
    parser.add_argument(
        "--rabbitmq-url",
    ...
    parser.add_argument(
        ...
        help="Stable Diffusion 1.5 request queue",
    )
    parser.add_argument(
        "--timeout", type=float, default=600.0, help="Response timeout in seconds"
    )
    ...
    parser.add_argument(
        "--output", type=Path, help="Optional generated PNG output path"
    )
    parser.add_argument(
        "--display",
        action="store_true",
