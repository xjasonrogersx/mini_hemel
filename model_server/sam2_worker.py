#!/usr/bin/env python3
"""Run an Ultralytics SAM2 segmentation worker behind RabbitMQ."""
...
class SAM2Worker:
    def __init__(self, args: argparse.Namespace) -> None:
        LOGGER.info("Loading Ultralytics SAM2 model: %s", args.model)
    ...
    def run(self) -> None:
        ...
        LOGGER.info("Waiting for SAM2 requests on queue %s", self.queue)
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        default="sam2_b.pt",
        help="Ultralytics SAM2 checkpoint or local model path",
    )
    parser.add_argument(
        "--queue",
        default="sam2",
        help="RabbitMQ request queue (default: sam2)",
    )
    parser.add_argument(
        "--rabbitmq-url",
